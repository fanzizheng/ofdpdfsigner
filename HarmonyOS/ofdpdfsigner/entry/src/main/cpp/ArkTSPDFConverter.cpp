...
napi_value PDFToImage(napi_env env, napi_callback_info info)
{
    ...
    bool isTF = pdf_toImage(filehandle,imageFormat.c_str(),scale,&outData,&outDatalen,&stroutDatalen);
    if ( !isTF ) {
        if ( outData != NULL ) {
    ...
    //----------------------------
    //OH_LOG_INFO(LOG_APP,"c++ pfd image lens %{public}s",stroutDatalen);
    ...
    for (itr = strlist.begin(); itr != strlist.end(); itr++) {
        ...
        if ( !tempstr.empty() ) {
            ...
            //OH_LOG_INFO(LOG_APP,"c++ pdf image len %{public}d,pos %{public}d ,i : %{public}d",len,pos,i);
...
napi_value PDFPageToImage(napi_env env, napi_callback_info info)
{
    ...
    bool isTF = pdf_pageToImage(filehandle,imageFormat.c_str(),pageNumber,scale,&outData,&outDatalen);
    if ( !isTF ) {
        if ( outData != NULL ) {
            FZZFreeHandle(outData);
            outData = NULL;
...
napi_value PDFToImageZip(napi_env env, napi_callback_info info)
{
    ...
    bool isTF = pdf_toImageZip(filehandle,imageFormat.c_str(),scale,&outData,&outDatalen);
    if ( !isTF ) {
        if ( outData != NULL ) {
            FZZFreeHandle(outData);
            outData = NULL;
...
napi_value PdfToOfd(napi_env env, napi_callback_info info)
{
    ...
    bool isTF = pdfToOfd(filehandle,!isIncludeStamp,&outData,&outDatalen);
    if ( !isTF ) {
        if ( outData != NULL ) {
            FZZFreeHandle(outData);
            outData = NULL;

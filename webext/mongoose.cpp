...
//fzz add begin
void mg_http_reply_longdata(struct mg_connection *c, int code, const char *headers,
                            const char *fmt, ...) {
  ...
  va_start(ap, fmt);
  va_copy(ap_copy, ap);
  datalen = vsnprintf(NULL, 0, fmt, ap_copy);
  va_end(ap_copy);

  buf = (unsigned char *)malloc(datalen + 1);
  if (buf == NULL) {
      va_end(ap);
      return;
  }
  vsnprintf((char *)buf, datalen + 1, fmt, ap);
  ...
  mg_printf(c, "HTTP/1.1 %d %s\r\n%sContent-Length: %-10lu \r\n\r\n", code,
            mg_http_status_code_str(code), headers == NULL ? "" : headers,datalen+1);

  // 第三步：写入正文
  mg_send(c, buf, datalen);

  // 清理临时 buffer（如果使用了动态内存）
  free(buf);
...
//fzz edit begin
#define MIP_TCP_KEEPALIVE_MS 240000  // TCP keep-alive period, ms
...
struct connstate {
 ...
 //fzz edit begin
  //#define MIP_TTYPE_KEEPALIVE 0
  #define MIP_TTYPE_KEEPALIVE 240000  // Connection is idle for long, send keepalive
      //fzz edit end

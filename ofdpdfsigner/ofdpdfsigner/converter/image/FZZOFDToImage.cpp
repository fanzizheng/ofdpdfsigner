...
/***********************************************FZZOFDToImage**************************************************************/
//-----------------------------------------------------------------------------------------------------------------
FZZOFDToImage::FZZOFDToImage() : m_reader(NULL),m_NoText(false),m_NoStamp(false),m_isAnnotation(false)
{
    //DLOG("FZZOFDToImage()");
    FZZConst::addClassCount("FZZOFDToImage");
    m_ppm = FZZCommonUtil::dpiToPpm(72);
}
//-----------------------------------------------------------------------------------------------------------------
FZZOFDToImage::FZZOFDToImage(FZZOFD * reader,double ppm) : FZZOFDToImage()
{
    m_ppm = ppm;
...
//-----------------------------------------------------------------------------------------------------------------
FZZOFDToImage::~FZZOFDToImage()
{
    //DLOG("~FZZOFDToImage()");
    FZZConst::delClassCount("FZZOFDToImage");
    
    m_pages = NULL;
    m_reader = NULL;
}
//-----------------------------------------------------------------------------------------------------------------
int FZZOFDToImage::pageSize()
{
    return m_pages == NULL ? 0 : (int)m_pages->size();
}
//-----------------------------------------------------------------------------------------------------------------
FZZBytes FZZOFDToImage::makePage(int pageIndex,string imageType,bool isNoText,bool isNoStamp)
{
    FZZBytes retBytes;
    ...
    fz_try(ctx) {
        imageUtil.CreateImage(pageWidthPixel,pageHeightPixel,0xFF,0xFF,0xFF,0xFF);
        ...
        if ( imageType == "PNG" ) {
            retBytes = imageUtil.PngEncode();
        } else if (imageType == "JPG") {
            retBytes = imageUtil.JPGEncode();
        }
    } fz_catch(ctx) {
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writePage(FZZImageUtil& graphics, TPageInfo* pageInfo,FZZMatrix & matrix,bool isNoText,bool isNoStamp)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writeStampAnnot( FZZImageUtil& graphics, FZZStampAnnotEntity* stampAnnotVo, FZZOFDStampAnnot* stampAnnot)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writeContent( FZZImageUtil& graphics, FZZOFDPageBlock* pageBlock, vector<FZZOFDDrawParam*>* drawParams,FZZMatrix & parentMatrix,bool isUsePreAlpha,int preAlpha,bool isNoText)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writeText( FZZImageUtil& graphics, FZZOFDTextObject* textObject, vector<FZZOFDDrawParam*>* drawParams,FZZMatrix & parentMatrix)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writeImage( FZZImageUtil& graphics, FZZOFDImageObject* imageObject, vector<FZZOFDDrawParam*>* drawParams,FZZMatrix & parentMatrix)
{
    if ( imageObject == NULL ) {
        return;
    }
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writePath( FZZImageUtil& graphics, FZZOFDPathObject* pathObject, vector<FZZOFDDrawParam*>* drawParams,FZZMatrix & parentMatrix,bool isUsePreAlpha,int preAlpha)
{
    if ( pathObject == NULL ) {
        return;
    }
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::writeComposite( FZZImageUtil& graphics, FZZOFDCompositeObject* compositeObject, vector<FZZOFDDrawParam*>* drawParams,FZZMatrix & parentMatrix,bool isNoText)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::addDrawParams(vector<FZZOFDDrawParam*>& drawParams, const string& refID)
{
    if ( !refID.empty() ) {
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::clipsAble(FZZImageUtil& graphics,FZZOFDClips * clips,FZZMatrix& clipsmatrix,double preboundary_x, double preboundary_y,int & clipscount)
{
    //ispop = false;
    if ( clips == NULL ) {
        return;
        ...
        for (Area_iter = areas->begin(); Area_iter != areas->end(); Area_iter++) {
            ...
            if (ct_path != NULL) {
                ...
                if ( ct_path->getFill() ) {
                    ...
                    //int aa = graphics.getWidth();
                    //int bb = graphics.getHeight();
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::clipsText(FZZImageUtil& graphics,vector<double> & ctm,FZZOFDText * ct_text,FZZMatrix& clipsmatrix,bool TransFlag)
{
    if ( ct_text == NULL ) {
        return;
    }
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::clipsPath(FZZImageUtil& graphics,vector<double> & ctm,FZZOFDPath * ct_path,FZZMatrix& matrix,double preboundary_x, double preboundary_y,bool TransFlag,/*out*/FZZPath& destpath)
{
    if ( ct_path == NULL ) {
        return;
    }
...
//-----------------------------------------------------------------------------------------------------------------
double FZZOFDToImage::getLineWidth(FZZOFDBaseGraphic* graphicUnit, vector<FZZOFDDrawParam*>* drawParams)
{
    if ( graphicUnit == NULL ) {
        return 0.1;
    }
...
//-----------------------------------------------------------------------------------------------------------------
TOFDColor* FZZOFDToImage::getStrokeColor(FZZOFDColor* color, FZZOFDColor* defaultColor, vector<FZZOFDDrawParam*>* drawParams)
{
    FZZOFDColor * c = color;
...
//-----------------------------------------------------------------------------------------------------------------
TOFDColor* FZZOFDToImage::getFillColor(FZZOFDColor* color, FZZOFDColor* defaultColor, vector<FZZOFDDrawParam*>* drawParams)
{
    FZZOFDColor * c = color;
    if ( c == NULL && drawParams != NULL ) {
        vector<FZZOFDDrawParam*>::const_iterator itr;
...
//-----------------------------------------------------------------------------------------------------------------
TOFDColor* FZZOFDToImage::getColor(FZZOFDColor* ctColor)
{
...
//-----------------------------------------------------------------------------------------------------------------
bool FZZOFDToImage::ShaderObject(FZZImageUtil& graphics,FZZOFDAxialShd * ct_AxialShd,string & strBoundary,double BoundaryBox_x, double BoundaryBox_y, double BoundaryBox_width, double BoundaryBox_height)
{
    bool isTF = false;
    if ( ct_AxialShd == NULL ) {
        return isTF;
...
//-----------------------------------------------------------------------------------------------------------------
bool FZZOFDToImage::PatternObject(FZZImageUtil& graphics,FZZOFDPattern * ct_Pattern,string & strboundary,double BoundaryBox_x, double BoundaryBox_y, double BoundaryBox_width, double BoundaryBox_height)
{
    bool isTF = false;
    if ( ct_Pattern == NULL ) {
        return isTF;
...
//-----------------------------------------------------------------------------------------------------------------
bool FZZOFDToImage::PatternImageObject(FZZImageUtil& graphics,FZZOFDImageObject* imageObject,string & preboundary,double preboundary_x, double preboundary_y,double BoundaryBox_width, double BoundaryBox_height,fz_rect view,fz_point pXY)
{
    bool isTF = false;
...
//-----------------------------------------------------------------------------------------------------------------
bool FZZOFDToImage::PatternTextObject(FZZImageUtil& graphics,FZZOFDTextObject* textObject,string& boundary, double BoundaryBox_width, double BoundaryBox_height,double xStep,double yStep)
{
    FZZMatrix matrix;
    double moveX = 0;
    double moveY = 0;
...
//-----------------------------------------------------------------------------------------------------------------
/*
bool FZZOFDToImage::PatternImageObject(FZZImageUtil& graphics,FZZOFDImageObject* imageObject,string & preboundary,double preboundary_x, double preboundary_y,double BoundaryBox_width, double BoundaryBox_height)
{
    bool isTF = false;
    if ( imageObject == NULL || m_reader == NULL ) {
        return isTF;
 ...
 */
//-----------------------------------------------------------------------------------------------------------------
FZZFontCache* FZZOFDToImage::getFont(FZZOFDTextObject* textObject)
{
...
//-----------------------------------------------------------------------------------------------------------------
void FZZOFDToImage::parseDelta(vector<string> & array,vector<double> & arr)
{
    if ( array.size() <= 0 ) {
        return;
    }
...
//-----------------------------------------------------------------------------------------------------------------
size_t FZZOFDToImage::getGlyphsSize(FZZOFDGlyphs * ofdglyphs)
{

...
//-----------------------------------------------------------------------------------------------------------------
FZZGBT35275PKCS9DSContainer::FZZGBT35275PKCS9DSContainer() : FZZExtendSignatureContainer(), m_PrivateKey(NULL),m_SignCert(NULL),m_isCallBack(false),m_signFunc(NULL),m_callBack_Param(NULL),m_getCertFunc(NULL)
{
    //DLOG("FZZGBT35275PKCS9DSContainer()");
    FZZConst::addClassCount("FZZGBT35275PKCS9DSContainer");
}
//-----------------------------------------------------------------------------------------------------------------
FZZGBT35275PKCS9DSContainer::FZZGBT35275PKCS9DSContainer(FZZKey * privateKey, FZZCert * signCert) : FZZGBT35275PKCS9DSContainer()
{
...
//-----------------------------------------------------------------------------------------------------------------
FZZGBT35275PKCS9DSContainer::FZZGBT35275PKCS9DSContainer(CallBack_Sign signFunc,CallBack_getUserCert getCertFunc,const void * callBack_Param) : FZZGBT35275PKCS9DSContainer()
{
    m_isCallBack = true;
    m_signFunc = signFunc;
...
//-----------------------------------------------------------------------------------------------------------------
FZZGBT35275PKCS9DSContainer::~FZZGBT35275PKCS9DSContainer()
{
    //DLOG("~FZZGBT35275PKCS9DSContainer()");
    FZZConst::delClassCount("FZZGBT35275PKCS9DSContainer");
    
}
//-----------------------------------------------------------------------------------------------------------------
void FZZGBT35275PKCS9DSContainer::sign(FZZBytes & inData, string propertyInfo,FZZBytes & outSignData)
{
    ...
    if ( m_isCallBack ) {
        ...
        m_signFunc(m_callBack_Param,plaintext.getData(),(int)plaintext.getLen(),&buffer,&bufferLen);
        if (bufferLen <= 0) {
            free(buffer);
            buffer = NULL;
            return ;
        ...
        signdata = gmCrypto.sign(*m_PrivateKey, plaintext);
    }
...
//-----------------------------------------------------------------------------------------------------------------
FZZOFDSigType::TYPE FZZGBT35275PKCS9DSContainer::getOFDSignType()
{
    return FZZOFDSigType::TYPE::TYPE_Sign;
}

//-----------------------------------------------------------------------------------------------------------------
string FZZGBT35275PKCS9DSContainer::getSignAlgOID()
{
    return "1.2.156.10197.1.501";
}
//-----------------------------------------------------------------------------------------------------------------
FZZBytes FZZGBT35275PKCS9DSContainer::getSeal()
{
    FZZBytes ret;
    return ret;
}

...
public:
    ...
    bool FZZKey(const Ch* str, SizeType len, bool copy) { return String(str, len, copy); }
...
public:
    ...
    bool FZZKey(Context& context, const Ch* str, SizeType len, bool) const {
        RAPIDJSON_SCHEMA_PRINT(Method, "Schema::FZZKey", str);
...
#define RAPIDJSON_STRING_(name, ...) \
    ...
    RAPIDJSON_STRING_(FZZErrorCode, 'e', 'r', 'r', 'o', 'r', 'C', 'o', 'd', 'e')
...
#define RAPIDJSON_SCHEMA_HANDLE_VALUE_(method, arg1, arg2) \
    ...
    bool FZZKey(const Ch* str, SizeType len, bool copy) {
        RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaValidator::FZZKey", str);
        if (!valid_) return false;
        AppendToken(str, len);
        if (!CurrentSchema().FZZKey(CurrentContext(), str, len, copy) && !GetContinueOnErrors()) {
            valid_ = false;
            return valid_;
        }
        RAPIDJSON_SCHEMA_HANDLE_PARALLEL_(FZZKey, (str, len, copy));
        valid_ = !outputHandler_ || outputHandler_->FZZKey(str, len, copy);
        return valid_;
    }

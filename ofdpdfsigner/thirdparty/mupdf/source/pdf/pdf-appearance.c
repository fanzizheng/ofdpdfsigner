...
//fzz add begin
fz_display_list *
pdf_signature_appearance_signed_ext(fz_context *ctx, fz_rect rect, fz_text_language lang, fz_image *img,float alpha,fz_image *showgraphic, const char *left_text, const char *right_text, int include_logo,fz_page *page,drawStampOverTextFunc drawStampOverText,drawStampOverText_EndFunc drawStampOverText_End)
{
	fz_display_list *dlist = NULL;
	fz_device *dev = NULL;
	fz_text *text = NULL;
	...
	fz_try(ctx)
	{
		...
		if (img)
		{
			...
			float scale = img_aspect > rect_aspect ? rectw / img->w : recth / img->h;
			fz_matrix ctm = fz_pre_translate(fz_pre_scale(fz_translate(midx, midy), scale * img->w, scale * img->h), -0.5, -0.5);
			fz_fill_image(ctx, dev, img, ctm, alpha, fz_default_color_params);

	...
	const char *location,fz_page *page,drawStampOverTextFunc drawStampOverText,drawStampOverText_EndFunc drawStampOverText_End)
{
	...
	fz_try(ctx)
	{
		if (!fz_is_empty_rect(rect))
		{
			info = pdf_format_signature_info(ctx, signer, flags, reason, location, now, &name);
			if (graphic)
				dlist = pdf_signature_appearance_signed_ext(ctx, rect, lang, graphic,alpha,showgraphic, NULL, info, logo,page,drawStampOverText,drawStampOverText_End);
			else if (flags & PDF_SIGNATURE_SHOW_GRAPHIC_NAME)
				dlist = pdf_signature_appearance_signed_ext(ctx, rect, lang, NULL,0,NULL, name, info, logo,page,drawStampOverText,drawStampOverText_End);
			else
				dlist = pdf_signature_appearance_signed_ext(ctx, rect, lang, NULL, 0,NULL,NULL, info, logo,page,drawStampOverText,drawStampOverText_End);
		}
		fz_rect mediabox;
    fz_matrix page_ctm;
    int is_horizontal = 0;
